...
function renderEntry(entry) {
  ...
  newHeader.className = "entry-header";

  const diarySpan = document.createElement("span");
  diarySpan.className = "entry-mood";
  diarySpan.textContent = MOOD_EMOJIS[entry.mood];

  const diaryH3 = document.createElement("h3");
  diaryH3.className = "entry-title";
  diaryH3.textContent = entry.title;
  ...
  newPtag.className = "entry-content";
  newPtag.textContent = entry.content;

  const newActions = document.createElement("div");
  newActions.className = "entry-actions";
  ...
  newHeader.appendChild(diarySpan);
  newHeader.appendChild(diaryH3);
  newHeader.appendChild(dateSpan);
  newDiv.appendChild(newPtag);
  newDiv.appendChild(newActions);
...
function updateEntry(id, updates) {
  ...
  const ent = diary.find(function (item) {
    return item.id === id;
  });
...
function fillFormForEdit(id) {
  ...
  const fillent = diary.find(function (item) {
    return item.id === id;
  });

  if (!fillent) return;

  titleInput.value = fillent.title;
  contentInput.value = fillent.content;
  moodSelect.value = fillent.mood;
  editingId = fillent.id;
...
function deleteEntry(id) {
  ...
  const delent = diary.findIndex(function (item) {
    return item.id === id;
  });
...
function renderStats() {
  ...
  const totalent = document.createElement("div");
  totalent.className = "stat-total";
  totalent.textContent = `전체 일기: ${stats.total}개`;

  const statsGrid = document.createElement("div");
  statsGrid.className = "stats-grid";

  statsContainer.appendChild(totalent);
  ...
  for (const mood in stats.moods) {
    ...
    statItem.textContent = `${MOOD_EMOJIS[mood]} ${mood}: ${count}개`;
